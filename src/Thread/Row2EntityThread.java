...
public class Row2EntityThread implements Runnable{
    public void run(){
        for (int i = 1;i<=10;i++){
                ...
                if (tile.isAdaTanaman()){
                    ...
                    for (Zombie z : tile.getZombieList()){
                        if (z instanceof CanJump){
                            ...
                            if (zj.alreadyJumped()==false && Map.getMapInstance().getMapDetail()[1][i-1].isAdaTanaman()==true){
                                zj.jump();
                                if (Map.getMapInstance().getMapDetail()[1][i-1].getPlant().isJumpable()==true){
                                    Map.getMapInstance().getMapDetail()[1][i-1].plantDie();
                                }
                                z.setTimeSpawn();
                                tile.getZombieList().remove(z);
                                Map.getMapInstance().getMapDetail()[1][i-1].getZombieList().add(z);
                            }
                        }
