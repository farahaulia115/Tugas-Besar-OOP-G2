...
public class Row3EntityThread implements Runnable{
    public void run(){
        for (int i = 1;i<=10;i++){
            ...
            Tile nextTile = Map.getMapInstance().getMapDetail()[2][i-1];
            Tile nextNextTile = null;
            if (i>=2){
                nextNextTile = Map.getMapInstance().getMapDetail()[2][i-2];
            }
            if (tile.isAdaTanaman()){
                    ...
                    if (tile.getPlant() instanceof Attack && tile.getPlant().getRange()==-1){
                        ...
                        for(int y = i;y<=9;y++){
                            Tile tileToAttack = Map.getMapInstance().getMapDetail()[2][y];
                        ...
                        if (i<10){
                            Tile tileToAttack = Map.getMapInstance().getMapDetail()[2][i+1];
                            if (tileToAttack.getZombieList().size()>0){
